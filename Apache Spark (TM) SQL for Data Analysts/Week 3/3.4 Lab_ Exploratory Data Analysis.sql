...
select count(quantity) from outdoorProducts where quantity < 0
...
select distinct countryName from sales
...
from salesQuants join countryCodes
on countryName = EnglishShortName
...
select totalQuantity, alpha3Code
from salesQuants join modCountryCodes
on countryName = EnglishShortName
